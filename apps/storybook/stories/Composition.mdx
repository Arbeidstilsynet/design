...
Rest props is a pattern where we collect all remaining props and pass them down to a child component or HTML element. This is often used to allow consumers to pass additional attributes like `id`, `className`, `style`, `aria-*` attributes, event handlers, etc.
...
Here is a real example of `FilePicker`'s props. Note that we don't need to explicitly define common props like `id`, `className`, `style`.
...
This is often done by creating a parent component that passes props down to child components via [context](https://react.dev/reference/react/createContext) as dependency injection.
...
Whenever we create a complex component, we should consider if it makes sense to break it down into subcomponents that can be used together.
